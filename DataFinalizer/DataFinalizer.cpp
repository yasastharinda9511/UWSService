...
void Finalizer::DataFinalizer::listen(Basics::BaseTraveller &base_data_traveller) {
    ...
    if (data_traveller != nullptr) {
        ...
        if (base_data_traveller.get_error_code() != 200) {
            _loop->defer([this, data_traveller]() {
                std::string response_json = _response_mappers.find(Constants::Events::ERROR)->second->handel_response(
                        *data_traveller);
...
Finalizer::DataFinalizer::DataFinalizer(uWS::Loop *loop) : _loop(loop) {

    _response_mappers.try_emplace(Constants::Events::ADD_STUDENT,
                                  new Finalizer::ResponseMapHandlers::AddStudentResponseHandler());
    _response_mappers.try_emplace(Constants::Events::GET_ALL_STUDENTS,
                                  new Finalizer::ResponseMapHandlers::GetAllStudentsResponseHandler());
    _response_mappers.try_emplace(Constants::Events::ERROR, new Finalizer::ResponseMapHandlers::ErrorHandler());
}

void Finalizer::DataFinalizer::set_event_loop(uWS::Loop *loop) {
    _loop = loop;
