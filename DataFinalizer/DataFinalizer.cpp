    ...
    if(data_traveller != nullptr){
        ...
        if(base_data_traveller.get_error_code() != 200) {
            _loop->defer([this, data_traveller](){
                std::string response_json = _response_mappers.find(Constants::Events::ERROR)->second->handel_response(*data_traveller);
        ...
        if(ite != _response_mappers.end()){
            ...
            if(_loop != nullptr){
                std::cout << "yasas tharinda !!!" << std::endl;
            }

//
            _loop->defer([&]() {
                auto response = data_traveller->get_response();
                std::cout << "yasas tharinda 234567!!!" << std::endl;
                auto x = ite->first;
